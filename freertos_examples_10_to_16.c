...
#define EXAMPLE_E1 (17)
#define EXAMPLE_E2 (18)
#define EXAMPLE_E3 (19)

#define TEST (EXAMPLE_E1)
...
// tengo una xqueue donde almaceno info y la leo en las tres tareas.
// void vSenderTask(*pvParameters)
// xQueueSendToBack envia en el primer param el dato del segundo, el tercero es el tiempo de bloqueo
...
#if (TEST == EXAMPLE_11)		/* Blocking when sending to a que
uctures on a queue */
...
#if (TEST == EXAMPLE_E1)		/* Using a counting semaphore to synchronize a task with an interrupt */
...
static void vtarea2(void *pvParameters);						// tomo el semaforo y sincronizo con tarea 3 con estructura
static void vPeriodicTask(void *pvParameters);					// activo una interrupcion  y envio semaforo a tarea 2
...
// tarea 2 consumidora/productora que necesita semaforo de int (Prioridad 3)
// esta funcion tiene que activar a la tarea 3 por una cola.
/* Take Semaphore, UART (or output) & LED toggle thread */
...
static void vtarea3(void *pvParameters)
{
	...
	while (1) {
		 ...
		 * task should remain in the Blocked state to wait for data to be available should
		 * the queue already be empty. */
		DEBUGOUT("La tarea 3 pide el dato.\r\n");
		xStatus = xQueueReceive(xQueue, &lReceivedValue, xTicksToWait);
		DEBUGOUT("La tarea 3 tiene el dato.\r\n");
		if (xStatus == pdPASS) {
			/* Data was successfully received from the queue, print out the received
			 * value. */
			DEBUGOUT("Received = %d\r\n", lReceivedValue);
...
static void vtarea2(void *pvParameters)
{
	...
	while (1) {
         ...
         * semaphore has been successfully obtained - so there is no need to check
         * the returned value. */
		DEBUGOUT("Dentro de tarea 2.\r\n");
        xSemaphoreTake(xBinarySemaphore, portMAX_DELAY);

        /* To get here the event must have occurred.  Process the event (in this
         * case we just print out a message). */
        DEBUGOUT("Handler task - Processing event.\r\n");
        DEBUGOUT("la tarea 2 envio el semaforo.\r\n");
        xStatus = xQueueSendToBack(xQueue, &lValueToSend, (portTickType)0);
...
// aca le entrego el semaforo a la tarea 2
void vSoftwareInterruptHandler(void)
{
	portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
...
int main(void)
{
    ...
    /* Check the semaphore was created successfully. */
		if (xBinarySemaphore != (xSemaphoreHandle) NULL) {
			 ...
			 * ensure it runs immediately after the interrupt exits.  In this case a
			 * priority of 3 is chosen. */
			xTaskCreate(vtarea2, (char *) "Tarea 2 ", configMINIMAL_STACK_SIZE, (void *) 200,
						(tskIDLE_PRIORITY + 2UL), (xTaskHandle *) NULL);
			...
			/* Create the task that will read from the queue.  The task is created with
			 * priority 2, so above the priority of the sender tasks. */
			xTaskCreate(vtarea3, (char *) "Tarea 3 receptor", configMINIMAL_STACK_SIZE, NULL,
						(tskIDLE_PRIORITY + 3UL), (xTaskHandle *) NULL);
...
#if (TEST == EXAMPLE_E2)		/* Using a counting semaphore to synchronize a task with an interrupt */
